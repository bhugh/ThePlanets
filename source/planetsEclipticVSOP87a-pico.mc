...
class VSOP87_cache{
    ...
    function fetch (now_info, timeZoneOffset_sec, dst, timeAdd_hrs, type) {
        ...
        var time_mod = Math.round(0.0 + timeAdd_hrs/24.0 + julianDate(now_info.year, now_info.month, now_info.day, 0, 0, timeZoneOffset_sec/3600.0, dst)).toNumber();
...
class vsop87a_pico{
   ...
   */
      ...
      function earth_z(t){
      return 0.00227822442 * Math.cos(3.41372504278 + 6283.07584999140*t)*t;
   }

   function earth_y(t){
        var y = 0.0;
        y = 0.00010466965 * Math.cos(0.09641690558 + 18849.22754997420*t);
  ...
  private function randomizeOrbit (name, EC, QR, W, IN, A, MA, d){
        ...
        var rFact = 2000;
        var div = 20000000; //was 10000000 but seemed too big, 100000000f but seemed too small
        
        //ret.put(:EC, pRet[:EC] + (Math.rand()%(2*rFact)-rFact) * QR);

        
        ret.put(:EC, pRet[:EC] + ((Math.rand()%(2*rFact)-rFact) * QR * eTime/3650 * QR/lastR)*A/div/100);
        ret.put(:QR, pRet[:QR] + ((Math.rand()%(2*rFact)-rFact) * QR * eTime/3650 * QR/lastR)*A/div);
        ret.put(:W, pRet[:W] + ((Math.rand()%(2*rFact)-rFact) * QR * eTime/3650 *A)/div);
        ret.put(:IN, pRet[:IN] + ((Math.rand()%(2*rFact)-rFact) * QR * eTime/3650 * QR/lastR*A)/div);
        ret.put(:A, pRet[:A] + ((Math.rand()%(2*rFact)-rFact) * QR * eTime/3650 * QR/lastR*A)/div);
